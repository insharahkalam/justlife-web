...
const Rating = () => {
  ...
  return (
    <section className='bg-[#F5F5F5] py-10 sm:py-16 w-full relative'>
      {/* Heading */}
      <div className='text-center pb-5 pt-10 sm:pt-15 px-4 sm:px-6 md:px-10'>
        <h1 className='text-2xl sm:text-3xl md:text-3xl font-bold'>What customers say about Justlife</h1>
        <p className='mt-3 sm:mt-5 text-base sm:text-lg md:text-lg'>
          Justlife has been rated 4.8 out of 5 based on 1626 reviews as of March 2024.
        </p>
      </div>

      {/* Stars */}
      <div className='w-[150px] sm:w-[175px] flex gap-1 justify-center items-center py-2 bg-white rounded-full relative top-5 z-10 shadow-sm mx-auto'>
        {[...Array(5)].map((_, i) => (
      ...
      {/* Review Card */}
      <div className="relative w-full max-w-[850px] sm:max-w-[850px] mx-auto -mt-1 overflow-visible px-4 sm:px-0">
        <div
          className={`relative bg-white rounded-2xl px-4 sm:px-15 py-10 sm:py-14 transition-transform duration-300 ${animate
              ? direction === "next"
        ...
        >
          <p className='text-base sm:text-xl md:text-2xl lg:text-3xl text-gray-700 text-center'>
            {reviews[index].text}
          </p>
          <h2 className='mt-6 sm:mt-20 text-center text-xl sm:text-3xl text-sky-400 font-medium'>
            {reviews[index].name}
          </h2>
        </div>
      </div>
      ...
      {index > 0 && (
        <button
          onClick={handlePrev}
          className='absolute left-4 sm:left-[35px] top-1/2 transform -translate-y-1/2 bg-white rounded-full shadow-md p-2 sm:p-3 hover:bg-gray-100 transition'
        >
      ...
      {index < reviews.length - 1 && (
        <button
          onClick={handleNext}
          className='absolute right-4 sm:right-[35px] top-1/2 transform -translate-y-1/2 bg-white rounded-full shadow-md p-2 sm:p-3 hover:bg-gray-100 transition'
        >
          <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000">
